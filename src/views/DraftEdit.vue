...
onMounted(async () => {
  ...
  if (draftId) {
    try {
      ...
      console.warn('API 加载失败，尝试从本地存储读取:', e.message)
      
      // 尝试从 localStorage 读取草稿
      ...
      if (localDraft) {
        ...
        if (localDraft.keyword) {
          ...
          keywords.value = Array.isArray(localDraft.form.keywords) 
            ? localDraft.form.keywords 
            : [localDraft.form.keywords]
        }
        
        // 设置编辑器内容
        editor.value?.commands.setContent(localDraft.content || '')
        ElMessage.warning('从本地缓存加载草稿')
      } else {
  ...
  try {
    ...
    if (draft.value.id) {
      ...
      try {
        ...
        // 创建失败，保存到 localStorage
        const localData = getList('drafts')
        const localId = draft.value.localId || Math.max(...localData.map(d => d.id), 0) + 1
        draft.value.localId = localId
        
        const existingIndex = localData.findIndex(d => d.id === localId)
        if (existingIndex > -1) {
          localData[existingIndex] = { ...localData[existingIndex], ...draft.value }
        } else {
          localData.push(draft.value)
        }
        saveList('drafts', localData)
        ElMessage.warning('已保存到本地缓存')
      }
    }
  } catch (e) {
